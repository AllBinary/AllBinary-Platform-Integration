...
//		keyRepeatTimer = new Timer();
	...
	private boolean commonKeyPressed(KeyEvent ev) {
            //System.out.println("commonKeyPressed: " + ev.keyCode);
		final int keyCode = ev.keyCode;
		if (inputMethodListener == null) {
                    //System.out.println("commonKeyPressed: no listener");
			...
			case '#':
				...
				break;
                        ...
                        //System.out.println("SwtInputMethod::keyPressed " + midpKeyCode);
		...
		if (gameKeyActionCode == Canvas.LEFT || gameKeyActionCode == Canvas.RIGHT) {
			synchronized (this) {
				if (gameKeyActionCode == Canvas.LEFT && caret > 0) {
					caret--;
				}
				if (gameKeyActionCode == Canvas.RIGHT && caret < inputMethodListener.getText().length()) {
					caret++;
				}
				lastButton = null;
		...
		if (keyCode == SWT.BS) {
                    //System.out.println("commonKeyPressed: Backspace");
		...
		if (keyCode == SWT.DEL) {
                    //System.out.println("commonKeyPressed: Delete");
			String tmp = inputMethodListener.getText();
			synchronized (this) {
				if (lastButton != null) {
					lastButton = null;
...
//			keyRepeatTimer.schedule(new KeyRepeatTask(), 50);
			...
			switch (ev.keyCode) {
                        ...
                        case SWT.PAGE_DOWN:
                            ...
                            break;
			default:
                            midpKeyCode = ev.keyCode;
			}

                        //System.out.println("SwtInputMethod::keyReleased " + midpKeyCode);
			da.keyReleased(midpKeyCode);
		}
	}
