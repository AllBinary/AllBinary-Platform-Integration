...
LRESULT wmKeyDown2 (long /*int*/ hwnd, long /*int*/ wParam, long /*int*/ lParam) {
	
    //System.out.println("wmKeyDown: " + wParam);
...
LRESULT wmKeyUp (long /*int*/ hwnd, long /*int*/ wParam, long /*int*/ lParam) {
    ...
    //System.out.println("wmKeyUp: " + wParam);
...
public static int[] isMouseButtonDown = new int[6];
LRESULT wmLButtonDown (long /*int*/ hwnd, long /*int*/ wParam, long /*int*/ lParam) {
    
    //System.out.println("Widget - wmLButtonDown: " + System.currentTimeMillis());
	Display display = this.display;
	LRESULT result = null;
	...
	if (count == 1 && (state & DRAG_DETECT) != 0 && hooks (SWT.DragDetect)) {
		if (!OS.IsWinCE) {
			...
			*/
			...
			if (isDisposed ()) { 
                            //System.out.println("wmLButtonDown: disposed"); 
                            return LRESULT.ZERO;
                        }
                        //System.out.println("wmLButtonDown: key/button state"); 
			mouseDown = OS.GetKeyState (OS.VK_LBUTTON) < 0;
		}
	}
	display.captureChanged = false;
        isMouseButtonDown[0] = 1;
	...
	if (dispatch && (consume == null || !consume [0])) {
            //System.out.println("wmLButtonDown: dispatch"); 
		result = new LRESULT (callWindowProc (hwnd, OS.WM_LBUTTONDOWN, wParam, lParam));	
	} else {
            //System.out.println("wmLButtonDown: no dispatch"); 
		result = LRESULT.ZERO;
	}
	if (OS.IsPPC) {
		/*
		...
		*/
		...
		if (hasMenu || hooks (SWT.MenuDetect)) {
                    //System.out.println("wmLButtonDown: winCE"); 
	...
	if (mouseDown) {
            //System.out.println("wmLButtonDown: down"); 
		if (!display.captureChanged && !isDisposed ()) {
                    //System.out.println("wmLButtonDown: no capture change and not disposed"); 
			if (OS.GetCapture () != hwnd) { 
                            //System.out.println("wmLButtonDown: capture is not current window");
                            OS.SetCapture (hwnd);
                        }
		}
	}
	if (dragging) {
            //System.out.println("wmLButtonDown: sendDragEvent0"); 
		sendDragEvent (1, x, y);
	} else {
            //System.out.println("wmLButtonDown: sendDragEvent1"); 
		if (detect != null && detect [0]) {
			...
			*/
			if (OS.GetKeyState (OS.VK_ESCAPE) >= 0) {
                            //System.out.println("wmLButtonDown: sendDragEvent up"); 
...
LRESULT wmLButtonUp (long /*int*/ hwnd, long /*int*/ wParam, long /*int*/ lParam) {
    //System.out.println("wmLButtonUp");
	Display display = this.display;
	LRESULT result = null;
        isMouseButtonDown[0] = 0;
...
LRESULT wmPaint (long /*int*/ hwnd, long /*int*/ wParam, long /*int*/ lParam) {
    ...
    //System.out.println("Widget - wmPaint: " + System.currentTimeMillis());
