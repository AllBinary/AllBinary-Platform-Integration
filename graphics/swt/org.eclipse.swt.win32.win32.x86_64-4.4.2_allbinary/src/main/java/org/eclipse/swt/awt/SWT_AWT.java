...
static synchronized void loadLibrary () {
	...
	*/
	...
	} catch (Throwable e) {final String message = ExceptionUtil.getInstance().getStackTrace(e); System.out.println("Exception in SWT: " + message);}
 ...
 * @since 3.0
 */
//private static String RUN_NOW = "run now";
//private static String RUN_LATER = "run later";
public static Frame new_Frame (final Composite parent) {
			...
			try {
				...
				try {
					...
					value = constructor.newInstance (new Object [] {new Integer ((int)/*64*/handle)});
                                    ...
                                    final String message = ExceptionUtil.getInstance().getStackTrace(e1); System.out.println("Exception in SWT: " + message);
					try {
						constructor = clazz.getConstructor (new Class [] {long.class});
						value = constructor.newInstance (new Object [] {new Long (handle)});
					} catch (Throwable e2) {
                                            final String message2 = ExceptionUtil.getInstance().getStackTrace(e2); System.out.println("Exception in SWT: " + message2);
						exception[0] = e2;
						return;
					}
				}
	...
	if (EventQueue.isDispatchThread() || parent.getDisplay().getSyncThread() != null) {
            //System.out.println(RUN_NOW);
		runnable.run();
	} else {
            //System.out.println(RUN_LATER);
		...
		while (result[0] == null && exception[0] == null) {
			...
			try {
				synchronized (result) {
                                    //System.out.println("result wait 50");
					result.wait(50);
				}
                                //System.out.println("result resume");
			} catch (InterruptedException e) {
                            final String message = ExceptionUtil.getInstance().getStackTrace(e); System.out.println("InterruptedException");
				interrupted = true;
			}
		}
		if (interrupted) {
