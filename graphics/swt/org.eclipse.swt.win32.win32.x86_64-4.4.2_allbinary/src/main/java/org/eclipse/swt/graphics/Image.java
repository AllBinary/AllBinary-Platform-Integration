...
//    private static final String INIT_HANDLE = "image init handle: ";
//    private static final String HANDLE = "image handle: ";
//    private static final String ICON_HANDLE = "image icon handle: ";
//    private static final String HANDLE_TOTAL = "image related handle total: ";
//    private static final String DESTROY_HANDLE_TOTAL = "destroy image related handle total: ";
//    private static final int MANY_HANDLES = 25;
//    private static final int MAX_HANDLES = 30;
		...
		case SWT.IMAGE_COPY: {
			switch (this.type) {
				case SWT.BITMAP:
					...
					this.handle = OS.CreateCompatibleBitmap(hdcSource, rect.width, bm.bmBits != 0 ? -rect.height : rect.height);
//                                        handleTotal++;
//                                        System.out.println(HANDLE + handleTotal);
//                                        if (handleTotal > MANY_HANDLES) {
//                                            new Exception().printStackTrace();
//                                            if (handleTotal > MAX_HANDLES) {
//                                                System.exit(0);
//                                            }
//                                        }
...
//                                                handleTotal++;
//                                                System.out.println(ICON_HANDLE + handleTotal);
//                                                if (handleTotal > MANY_HANDLES) {
//                                                    new Exception().printStackTrace();
//                                                    if (handleTotal > MAX_HANDLES) {
//                                                        System.exit(0);
						...
						*/
						...
						if (graphics != 0) {
							...
							if (status != 0) {
								...
								this.handle = 0;
//                                                                handleTotal--;
//                                                                System.out.println(HANDLE_TOTAL + handleTotal);
...
void destroy () {
	if (this.memGC != null) this.memGC.dispose();
	if (this.type == SWT.ICON) {
		if (OS.IsWinCE) this.data = null;
	...
	this.handle = 0;
//        handleTotal--;
//        System.out.println(DESTROY_HANDLE_TOTAL + handleTotal);
...
public boolean equals (Object object) {
	...
	return device == image.device && this.handle == image.handle;
}
...
public Color getBackground() {
	...
	if (this.transparentPixel == -1) return null;
...
public Rectangle getBounds() {
	...
	switch (this.type) {
		...
		case SWT.ICON:
			if (OS.IsWinCE) {
				return new Rectangle(0, 0, width = this.data.width, height = this.data.height);
			} else {
			...
			*/
			...
			if (OS.IsWinCE) {
				if (!isDib) {
					...
					if (mustRestore) {
						long /*int*/ hOldBitmap = OS.SelectObject(this.memGC.handle, this.handle);
						this.memGC.data.hNullBitmap = hOldBitmap;
					}
					isDib = true;
				}
			}
...
//        handleTotal++;
//        System.out.println(INIT_HANDLE + handleTotal);
//        if (handleTotal > MANY_HANDLES) {
//            new Exception().printStackTrace();
//            if (handleTotal > MAX_HANDLES) {
//                System.exit(0);
...
public long /*int*/ internal_new_GC (GCData data) {
	...
	*/
	if (this.type != SWT.BITMAP) {
                System.out.println("Type not Bitmap");
		SWT.error(SWT.ERROR_INVALID_ARGUMENT);
	}
        if (this.memGC != null) {
                System.out.println("memGC is not null");
		SWT.error(SWT.ERROR_INVALID_ARGUMENT);
	}
...
public void setBackground(Color color) {
	...
	*/
	...
	if (this.transparentPixel == -1) return;
	transparentColor = -1;
