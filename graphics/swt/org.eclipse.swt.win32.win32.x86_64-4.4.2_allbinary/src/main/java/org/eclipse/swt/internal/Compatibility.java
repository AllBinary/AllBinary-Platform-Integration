...
public static String getMessage(String key) {
	...
	if (msgs == null) {
		try {
			msgs = ResourceBundle.getBundle("org.eclipse.swt.internal.SWTMessages"); //$NON-NLS-1$
                    ...
                    final String message = ExceptionUtil.getInstance().getStackTrace(ex); System.out.println("Exception in SWT: " + message);
	...
	if (msgs != null) {
		...
		} catch (MissingResourceException ex2) {final String message = ExceptionUtil.getInstance().getStackTrace(ex2); System.out.println("Exception in SWT: " + message);}
	}
	return answer;
}
