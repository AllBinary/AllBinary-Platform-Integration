...
protected void syncExec (Runnable runnable) {
	...
	synchronized (lock) {
		...
		while (!lock.done ()) {
			try {
                            //System.out.println("wait: " + runnable);
				lock.wait ();
			} catch (InterruptedException e) {
                            final String message = ExceptionUtil.getInstance().getStackTrace(e); System.out.println("InterruptedException: " + message);
				interrupted = true;
			}
		}
                //System.out.println("resume: " + runnable);
		if (interrupted) {
